...
export const TrailCard: React.FC<TrailCardProps> = ({
  ...
  return (
    <Card
      onClick={onSelect}
      className={cn(
        "group relative mb-3 flex cursor-pointer gap-3 rounded-2xl border-transparent bg-[#020817]/95 px-4 py-3 text-slate-50 shadow-[0_14px_40px_rgba(15,23,42,0.7)] transition-all duration-200",
        "active:scale-[0.97] active:bg-[#020817]",
        isFavorite && "ring-1 ring-emerald-400/60"
      )}
    >
      <div className="flex min-w-0 flex-1 flex-col justify-between">
        ...
        <div className="mt-1 flex flex-wrap items-center gap-1.5 text-[11px] text-slate-400">
          ...
          <span className="mx-1 h-0.5 w-0.5 rounded-full bg-slate-500" aria-hidden="true" />
          <span
            className={cn(
      ...
      <div className="flex flex-col items-end justify-between gap-1">
        <Button
          ...
          className={cn(
            "relative h-11 w-11 rounded-full bg-slate-900/80 text-slate-400 shadow-lg transition-all duration-150",
            "active:scale-95",
